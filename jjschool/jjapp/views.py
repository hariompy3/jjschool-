...
from django.contrib.auth.decorators import login_required, user_passes_test
from django.contrib import messages
from .models import Teacher, TeacherSubjectAssignment, Grade, Subject, Student, Result
...
#def teacher_results_main(request):
#    teacher = request.user.teacher
#    assignments = TeacherSubjectAssignment.objects.filter(teacher=teacher)
#    context = {
#        'assignments': assignments,
#    }
#    return render(request, 'teacher/results_main.html', context)

#@login_required
#@user_passes_test(is_teacher)
#def list_class_results(request, assignment_id):
...
#    results = Result.objects.filter(student__in=students, subject=assignment.subject)
#    
#    context = {
#        'assignment': assignment,
#        'results': results,
#    }
#    return render(request, 'teacher/list_class_results.html', context)

#@login_required
#@user_passes_test(is_teacher)
#def add_edit_result(request, assignment_id, student_id=None):
#    assignment = get_object_or_404(TeacherSubjectAssignment, id=assignment_id, teacher=request.user.teacher)
#    student = get_object_or_404(Student, id=student_id, grade=assignment.grade) if student_id else None
#    result = Result.objects.filter(student=student, subject=assignment.subject).first() if student else None
...
#            new_result.subject = assignment.subject
#            new_result.save()
#            messages.success(request, 'Result saved successfully.')
#            return redirect('list_class_results', assignment_id=assignment_id)
#    else:
...
#        'student': student,
#    }
#    return render(request, 'teacher/add_edit_result.html', context)
#    
#    
#    
#    
...
#    
#    return render(request, 'student_list.html', {'students': list(students.values())})
#    
#    
#    
#    
...
from django.contrib import messages
from django.utils import timezone
from .models import Teacher, TeacherSubjectAssignment, Student, Result, Exam
from .forms import ResultForm
...
def teacher_assignments(request):
    ...
    assignments = TeacherSubjectAssignment.objects.filter(teacher=teacher, academic_year=academic_year)
    return render(request, 'teacher/assignments.html', {'assignments': assignments})

@login_required
@user_passes_test(is_teacher)
def assignment_exams(request, assignment_id):
    assignment = get_object_or_404(TeacherSubjectAssignment, id=assignment_id, teacher=request.user.teacher)
    exams = Exam.objects.filter(academic_year=assignment.academic_year)
    return render(request, 'teacher/assignment_exams.html', {'assignment': assignment, 'exams': exams})

@login_required
@user_passes_test(is_teacher)
def enter_results(request, assignment_id, exam_id):
    assignment = get_object_or_404(TeacherSubjectAssignment, id=assignment_id, teacher=request.user.teacher)
    exam = get_object_or_404(Exam, id=exam_id)
    ...
    results = {result.student_id: result for result in Result.objects.filter(exam=exam, assignment=assignment)}
    ...
    return render(request, 'teacher/enter_results.html', context)    
...
@login_required
@user_passes_test(is_teacher)
def manage_results(request, assignment_id):
    assignment = get_object_or_404(TeacherSubjectAssignment, id=assignment_id, teacher=request.user.teacher)
    students = Student.objects.filter(classroom=assignment.classroom)
    ...
    if request.method == 'POST':
        for student in students:
            ...
            result.marks = float(marks) if marks else None
            result.grade = grade
            result.comments = comments
            result.save()
    ...
    results = {result.student_id: result for result in Result.objects.filter(assignment=assignment)}
